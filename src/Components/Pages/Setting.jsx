...
export default function Setting({ sidebarExpanded, setSidebarExpanded }) {
  ...
  /* ================= AUTH - FIXED TO MERGE FIREBASE + FIRESTORE ================= */
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (snap.exists()) {
        ...
        // 🔥 MERGE Firebase user + Firestore data for Header
        const mergedUser = { 
          ...firebaseUser, 
          ...firestoreData,
        ...
        // Sync sound settings
        if (firestoreData.soundEnabled !== undefined) setSoundEnabled(firestoreData.soundEnabled);
        if (firestoreData.soundVolume !== undefined) setSoundVolume(firestoreData.soundVolume);
      } else {
  ...
  return (
    <div className="pt-[40px] flex h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black relative overflow-hidden">
      ...
      {/* SHARED HEADER - FIXED WITH USER DATA */}
      <Header 
        title="Settings" 
        sidebarExpanded={sidebarExpanded} 
        setSidebarExpanded={setSidebarExpanded}
        currentUser={user}  // 🔥 NOW PASSED - Header will show username!
      />
      ...
      <div className="flex-1 sm:ml-[92px] flex flex-col p-8 text-white overflow-hidden">
        ...
        {/* SCROLLABLE SETTINGS CONTENT - HIDDEN SCROLLBAR */}
            ...
            <div className="lg:col-span-1 xl:col-span-2 space-y-6">
              ...
              <section className="lg:col-span-1 xl:col-span-2 bg-gray-800/70 backdrop-blur-sm rounded-3xl border border-gray-700/50 overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-300">
                ...
                <div className="p-6 lg:p-8 space-y-6">
                  ...
                  {/* Sound Control */}
                  <div className="bg-gray-700/50 border border-gray-600/50 rounded-2xl p-8">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                      <div className="flex items-center gap-4 flex-1">
                        {soundEnabled ? <FaVolumeUp className="w-6 h-6 text-green-400" /> : <FaVolumeMute className="w-6 h-6 text-gray-500" />}
                        <div>
                          <div className="font-semibold text-base lg:text-lg">Reaction Sound</div>
                          <div className="text-gray-400 text-sm">Message reaction audio feedback</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <label className="relative inline-flex h-7 w-12 items-center rounded-full bg-gray-600 cursor-pointer transition">
                          <input
                            type="checkbox"
                            checked={soundEnabled}
                            onChange={(e) => saveSoundPreference(e.target.checked, soundVolume)}
                            className="sr-only peer"
                          />
                          <span className="w-6 h-6 mx-0.5 rounded-full bg-white shadow-sm translate-x-0.5 transition peer-checked:translate-x-6 peer-checked:bg-green-500" />
                        </label>
                        <input
                          type="range"
                          min="0"
                          ...
                          step="0.01"
                          value={soundVolume}
                          onChange={(e) => saveSoundPreference(soundEnabled, parseFloat(e.target.value))}
                          className="w-40 lg:w-56 h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-green-500 hover:accent-green-400 transition flex-shrink-0"
                        />
                      </div>
                    </div>
                  </div>
